...
def _leak_decoy_env(decoy_repo: Path) -> dict[str, str]:
    ...
    the LOD-314 regression tests to simulate gdmutant being invoked from inside a git hook."""
    return {
...
def test_has_uncommitted_changes_ignores_leaked_hook_git_env(
    ...
    """Regression for LOD-314: production `_has_uncommitted_changes` (the ``git status
...
def test_changed_lines_ignores_leaked_hook_git_env(
    ...
    """Regression for LOD-314: production `_changed_lines` (``git diff --unified=0`` /
...
def test_all_git_subprocess_calls_scrub_leaked_hook_env(
    ...
    """Regression for LOD-314, direct unit-level pin: every git subprocess.run call gdmutant makes
