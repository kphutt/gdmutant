...
def _expand_sources(paths: list[str], exclude: list[str] | None = None) -> list[str] | None:
    ...
    `exclude` glob (LOD-204), noting how many of each (counted per *unique* file, so duplicate dir
    ...
    — the "point it at a directory" adoption case (LOD-79). Existence/validity of each file is
...
def build_parser(config: dict[str, object] | None = None) -> argparse.ArgumentParser:
    ...
    # (argparse precedence: passed value > set_defaults > add_argument default). LOD-107. `exclude`
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        ...
        # (both narrow a directory target; neither can drop an explicitly named file). LOD-204.
