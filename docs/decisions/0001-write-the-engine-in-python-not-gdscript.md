...
question, asked deliberately rather than assumed away, is whether to write the tool *in* GDScript,
...
- The parser only exists in Python. [`gdtoolkit`](https://github.com/Scony/godot-gdscript-toolkit)
  ...
  within GDScript, because its real parser is C++, internal to the editor. So a GDScript
...
- Write the engine and the GDScript adapter in Python. Mutation is a direct `gdtoolkit`
  ...
  `pyproject.toml` (no executable `setup.py`), with the toolchain pinned via `uv` (`uv.lock`,
  `.python-version`).
- The adapter runs the tests as a subprocess, `godot --headless` plus GdUnit4 (the chosen
  first test-runner), parsing its machine-readable output. Godot is an unavoidable prerequisite in
  *any* design, because it is what runs the tests. Only the orchestration and mutation live in
  Python.
- Keep the engine language-neutral. No GDScript-specific assumptions leak into `engine/`, and
  language specifics live only in `adapters/gdscript/`.

## Consequences
- The one real cost is a second runtime for Godot devs. Mitigation: keep prerequisites to
