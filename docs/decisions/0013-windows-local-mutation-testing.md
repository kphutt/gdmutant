...
## Status
Accepted, corrected 2026-07-31: the Context misstates *why* mutmut reaches none of these method
bodies. The gap and the decision both stand. See [Correction](#correction-2026-07-31).
...
documents the resulting blind spot, where `Mutant.apply`, `Span.__post_init__`, the `GdUnit4Runner`
and `CommandRunner` methods, and others are unit-tested but never *mutation-measured*. This isn't
...
is open, and the draft fix ([#404][404]) was abandoned by its own author: reusing worker processes across
...
It works, but measured on this machine it's architecturally much slower than mutmut per mutant, since every
mutant pays a fresh subprocess start, import, and test-collection cost that mutmut's fork model skips. Worse,
...
interrupted or backgrounded `cosmic-ray` run left a mutation applied and unreverted in
`gdmutant/engine/spans.py`, on disk, in the real working tree, twice. Git Bash's process tools
(`ps`/`jobs`/`kill`) couldn't even see the native Windows processes responsible, and `tasklist` plus
`taskkill /F` was required to stop them.
...
(`engine/mutants.py`) found 21 mutants (against ADR-0008's 20), 16 caught, 5 surviving, including
...
the suite from *that* copy, so the real source tree is never the mutation target and a hard kill can't
corrupt it by construction rather than by discipline. (This is also what makes it viable as a routine hook
...
pytest or python processes were left running.
...
pre-push hook. Diff-scoping mutation testing is the mainstream answer to this exact tradeoff, used by
Google (["State of Mutation Testing at Google"][google-mt], ICSE-SEIP 2018) and by Stryker
([`--since`][stryker-since]).
...
- Two mutation *tools* now exist in the repo (mutmut for CI, poodle for local) instead of one. They
  will not produce identical scores on the same file, because their operator sets differ, and that's
  accepted: the local run's job is "did my change just break test coverage," not "match CI's number."
- poodle is a small, single-maintainer project (~1,500 downloads/month), a real supply-chain tradeoff
  next to mutmut's much larger install base. Accepted for a dev-only, advisory tool, and worth
  revisiting if it goes quiet the way `mutatest` did (evaluated and rejected here for exactly that
  reason, since it was last released 2022-02 and is incompatible with 3.11+ bytecode).
...
- The named risk this ADR carries: [ADR-0008][adr8] tried a manual method-body check on 2026-07-19
  ...
  just in tool, because it's a named, invocable pre-commit hook (`gdmutant-mutation`) rather than a
  "run this by hand" instruction, but that alone doesn't guarantee it gets used. The trigger that's
  supposed to make it get run is already this operator's own definition-of-done for advisory
  mutation checks: on any new or changed pure logic, run it before merge and drive survivors to zero.
  If this hook is still going unused at the next review, the honest move is the one considered and
  rejected tonight: drop the local leg, accept the class-method gap as documented-but-unmeasured,
  same as mutmut's own scope note already does for the rest of it.

## Correction (2026-07-31)

The Context opens on a claim about mutmut that this record inherited from [ADR-0008][adr8], that it
...
The gap itself is unchanged, and so is this decision. Re-measured on 2026-07-31: mutmut generates
3,773 mutants across `gdmutant/` and 0 of them land in a class-method body, which is the same
condition this ADR was written to act on. A local poodle hook is still the thing that reaches those
bodies. And the Windows half of the argument never depended on the misstated reason at all, because
mutmut cannot run on this machine whatever it would mutate if it could.
