...
1. Line scope is too coarse. A single line often holds mutants from *different* operators. For
   example, `if value < 0:` yields a `comparison` mutant (`<`→`<=`) *and* two `numeric` mutants
   (`0`→`1`, `0`→`-1`). Across that project's modules, every equivalent survivor shared its line with
   a *killed* or *timeout* mutant, so line-scoped suppression would have hidden genuine coverage. The
...
- Operator scope. `# gdmutant: ignore[<operator>]` suppresses only the named operator's
  mutant(s) on the line (`mutatorName` in the report), and several can be comma-listed as
  `ignore[comparison, numeric]`. A bare `# gdmutant: ignore` keeps its 0004 meaning: every operator
  on the line (backward-compatible). Scope is still the *physical line*, like `# noqa` and
  `# type: ignore[code]`.
- Reason. Trailing text after the marker or brackets is the human reason:
...
- Surface, don't drop, reversing 0004's rejected option (c). A suppressed mutant is still
  *generated* but never run. It is classified `Ignored` (Stryker's `Ignored` `MutantStatus`),
  ...
  covers the benign or brittle-to-kill case (a reasoned `Ignored`, distinct from a proven
  equivalent) without a separate status.
- Where it lives is unchanged from 0004: the adapter, since `# …` is GDScript comment syntax. The
  engine stays language-neutral (a generic `Mutant.ignore_reason` the adapter sets).
- Typo guard. An `ignore[<name>]` naming an operator no catalog produces suppresses nothing, and the
