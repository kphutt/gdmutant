...
existing operator, which is a token→token swap that stays valid GDScript, deleting a statement can
produce source that parses but does not compile. Concretely, deleting the `return` from a *typed*
...
killed) and the GdUnit4 runner write no report (→ `error`, excluded). That is the same mutant with a
...
Options considered. (1) Emit everything and accept the timeout and error noise. (2) Delete only
...
- the function body's last top-level statement is a *different* `return`, because a guaranteed final
  return backstops the deletion, so every path still returns a value.

Otherwise (a typed function's sole or final return, or one whose last top-level statement isn't a
...
via `--check-only`). Declarations (`func_var_stmt`) are deferred, because deleting one either breaks
a later reference or is an equivalent, and both are noise. Multi-line statements are skipped
(`spans.py` edits a single line). This mirrors the adapter's existing `_string_format_percents`
precedent: tree-aware suppression of a known-broken mutant class, in the conservative direction.
...
- The guard embeds a *partial* model of Godot's flow analysis. The failure mode to watch is
  ...
  dedicated adversarial `--check-only` self-test exercises the path the corpus lacks), and the
  remaining candidates are `match` exhaustiveness and a future Godot tightening. The `--check-only`
  oracle in the self-test is exactly the check that would catch it. If it ever fails, tighten the
  rule or narrow to expression-only.
- In fully-typed code, a function with a sole `return` gets no deletion mutant. Accepted for v0.1,
  and richer flow analysis (or an opt-in "aggressive" mode) is future work.

Decision reached via a structured design-review method on the design fork (grounded against the real
corpus, runners, and Godot). Independent follow-up: harden the corpus harness to `quit()` on a load
error so a *future* broken script fails fast rather than hanging (a CommandRunner-adoption gotcha).
