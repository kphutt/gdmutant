...
## Status
Accepted, corrected 2026-07-31: the last Consequences bullet has been overtaken by events. The
decision itself stands. See [Correction](#correction-2026-07-31).
...
public before the first tagged release, so this ADR records the publishing mechanism as launch-prep,
...
- (a) A long-lived API token stored as a GitHub Actions secret. Simple, but it is a standing
  bearer credential: anyone (or any compromised workflow or action) that can read the secret can
  publish as us, indefinitely, until it is manually rotated. It is exactly the kind of long-lived
  secret the maintainer's supply-chain baseline exists to avoid, and it must be seeded, stored, and
  rotated by hand.
...
`pypa/gh-action-pypi-publish`. No PyPI or TestPyPI API token is stored as a repo or org secret.

### Trigger design (two paths in one workflow)
- `workflow_dispatch` -> TestPyPI, the deliberate dry-run. Fired by hand from the Actions tab, it
  builds and publishes to `https://test.pypi.org/legacy/`. Lets us rehearse the real thing against
  the throwaway index without cutting a release.
- `release: types: [published]` -> PyPI, a real release. Publishing a GitHub Release runs the
  PyPI path. Tag and release management stays the human, auditable act that gates a real upload.

### Least-privilege shape
- Workflow-level `permissions: contents: read`. The build job (which runs the build backend's
  code via `uv build`) inherits only that, so it has no OIDC.
- `id-token: write` is granted only on the two publish jobs, and each publish job is bound to a
  ...
  branch or tag constraints) from the repo settings.
- Build once, publish from an artifact: the OIDC-bearing publish jobs do nothing but download the
  vetted `dist/` artifact and upload it, so the build backend never executes in a job that can mint
  a credential.
- Every action is SHA-pinned to a full commit with a `# vX.Y.Z` comment (repo convention, and
...
upload. This is a web-login action only the account owner can do, and it is the single irreducible
manual step. Register the same identity on both indexes:
...
## Consequences
- No long-lived publishing secret in the repo, ever. Nothing to leak or rotate, and a compromised
...
- The seed is a hard prerequisite. Until the maintainer registers the pending publishers (above),
  ...
  is by design, because the trust is established out-of-band, on the index, by a human.
- TestPyPI is a genuine rehearsal. `workflow_dispatch` exercises the real OIDC and upload path
  against the throwaway index, so the first real PyPI release is not the first time the mechanism
  runs end-to-end.
...
- Version management is out of scope here. This ADR is the publishing *mechanism*, and the workflow
  publishes whatever version the built distribution declares. The bullet's original statement about
  the value of the `version` field has since been overtaken, see
  [Correction](#correction-2026-07-31).

## Correction (2026-07-31)

The last Consequences bullet originally read:

> the `version` field in `pyproject.toml` stays `0.0.0` until a separate version-cut

That was accurate when it was written and is no longer. The field reads `0.1.0`, and has done since
the version cut landed later the same day, in a change about tidying the repository root rather than
about publishing. The follow-up that would have carried the matching edit to this record was dropped
before anyone worked it, which is the whole reason the line survived here for as long as it did.

Nothing about the decision changes. Trusted Publishing never reads the version field, and the
surrounding sentence still holds: this record covers the publishing mechanism, and the workflow
uploads whatever version the built distribution declares. Read the current value from
`pyproject.toml`, never from here.
