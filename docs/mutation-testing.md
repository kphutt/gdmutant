...
**691 / 709 mutants killed — the remaining 18 are equivalent mutants** (changes that cannot alter
...
### Scope: what the 709 covers

mutmut 3.6 mutates **module-level functions only** — it does not generate mutants for class-method
bodies. So the 709 spans the package's 28 module-level functions (the operator catalog, spans, mutant
