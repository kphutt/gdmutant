...
**671 / 689 mutants killed — the remaining 18 are equivalent mutants** (changes that cannot alter
...
### Scope: what the 689 covers

mutmut 3.6 mutates **module-level functions only** — it does not generate mutants for class-method
bodies. So the 689 spans the package's 27 module-level functions (the operator catalog, spans, mutant
