...
**545 / 562 mutants killed — the remaining 17 are equivalent mutants** (changes that cannot alter
...
### Scope: what the 562 covers

mutmut 3.6 mutates **module-level functions only** — it does not generate mutants for class-method
bodies. So the 562 spans the package's 24 module-level functions (the operator catalog, spans, mutant
