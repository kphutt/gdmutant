...
**501 / 518 mutants killed — the remaining 17 are equivalent mutants** (changes that cannot alter
...
### Scope: what the 518 covers

mutmut 3.6 mutates **module-level functions only** — it does not generate mutants for class-method
bodies. So the 518 spans the package's 22 module-level functions (the operator catalog, spans, mutant
...
covered by unit tests but are not *mutation-measured* here — so read the score as "every behavioral
