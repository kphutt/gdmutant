...
([internal-tool]/mutation split) — this is the standalone continuation.

## NEXT STEPS (for a fresh session picking this up cold)
1. **Harden the repo** — run the house baseline (`[dev-path] §1
...
## Relationship to [internal-tool] (a peer, not a parent)
[internal-tool] (the grounded PR reviewer, in `[internal-tool]/prompts/[internal-tool]/`) and this tool are **two advisory signals
in the same merge gate** — *"did we build the right thing?"* ([internal-tool]) vs *"do the tests actually bite?"*
